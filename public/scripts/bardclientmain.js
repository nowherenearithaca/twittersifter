...
/* jshint ignore:start */
/**
 * Created by bradflyon on 1/16/15.
 */
...
$(document).ready(function() {
    ...
    var theClipboard = new Clipboard('.btn.copy', {
                text: function(trigger) {
                    return trigger.getAttribute('data-clipboard-text');
                }
            });

    theClipboard.on('success', function(event) {
        event.clearSelection();
        var original = event.trigger.textContent;
        event.trigger.textContent = 'Copied!';
        window.setTimeout(function() {
            event.trigger.textContent = original; //'Copy';
        }, 250);
    });

    var uri = new URI.parseQuery(window.location.search);

    console.dir(uri);

    var showTest = (typeof uri.showTest !== "undefined");

    bardClient = new BardClient({showTest: showTest});

    var togglePause = function() {
        var isPaused = bardClient.getIsPaused();
        bardClient.setIsPaused(!isPaused); //
    };
     ...
     $('#do-colorify').click(function() {

        var doColorify = $(this).is(':checked');
        bardClient.setDoColorify(doColorify);

     });

    bardClient.setDoColorify($('#do-colorify').is(':checked'));


     $('#do-show-user-info').click(function() {

        var b = $(this).is(':checked');
        bardClient.setDoShowUserInfo(b);

     });

     $('#do-show-user-references').click(function() {

        var b = $(this).is(':checked');
        bardClient.setDoShowUserReferences(b);

     });

    bardClient.setDoShowUserReferences($('#do-show-user-references').is(':checked'));


     $('.email-picked-tweets').click(function() {
        bardClient.doEmailPickedTweets();        
     });

    bardClient.setDoShowUserInfo($('#do-show-user-info').is(':checked'));

     $('#txtSearchTerms').keypress(function(e) {
         if (e.which === 13) {
             //TODO - implement this if api will let it change... restartWithTerms();
         }
     });

    $('#number-followers-to-interrupt').on('change', function() {
       bardClient.setFollowerThresholdToInterrupt(this.value); // or $(this).val()
    });

    $('#min-diff-between-handling-tweets-ms').on('change', function() {
       bardClient.setMinDiffBetweenHandlingTweets_ms(this.value); // or $(this).val()
    });

    $('#interest-threshold').on('change', function() {
    ...
    var interestThresholdSlider =  $('#interesting-tweet-threshold-slider')
                                    .slider({
                                        formatter: function(value) {
                                            return 'Interest threshold: ' + Number(value).toFixed(1);
                                        }
                                    })
                                    .data('slider')                                    
                                    .on('slide', function() {
                                        var value = Number(interestThresholdSlider.getValue()).toFixed(1);
                                        currentInterestThreshold.html(value);
                                        bardClient.setInterestThreshold(value); 
                                    });




    bardClient.setFollowerThresholdToInterrupt($('#number-followers-to-interrupt').val());
    bardClient.setMinDiffBetweenHandlingTweets_ms($('#min-diff-between-handling-tweets-ms').val());

    bardClient.setInterestThreshold(interestThresholdSlider.getValue());// $('#interest-threshold').val()); // or $(this).val()
    currentInterestThreshold.html(Number(interestThresholdSlider.getValue()).toFixed(1));


    //var heartYou = new HeartYou({svgElementID:"heart-area"});
    //bardClient.setHeartYou(heartYou);
    // //set up toggle for say it or not...
    // $("#set-enable-speak").on("click", function() {

    //     //get value
    //     var theValue = true; //get this from the checkbox...
    //     bardClient.setDoSayTweet(theValue);
